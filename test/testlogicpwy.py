...
class testclean(unittest.TestCase):
    def setUp(self):
        ...
        self.test_obj.units[100] = unit.UnitObject(100, 0, 'meat', (1, 1), test_buff)  # 加入一个编号为0的单位
        self.test_obj.units[101] = unit.UnitObject(101, 0, 'eagle', (1, 1), test_buff)

    def test_clean(self):
        self.test_obj.units[100].health_now = 0  # 生命值置零情况
        people = self.test_obj.resource[0]['remain_people']
        self.test_obj.cleanup_phase()
        id_collection = list(self.test_obj.units)
        self.assertTrue(not(100 in id_collection))  # 删除之后检查是否还在units中
        self.assertTrue(people == self.test_obj.resource[0]['remain_people'] - origin_attribute['meat'][
            'people_cost'])  # 检查人口是否变化正确
        self.test_obj.units[101].hacked_point = self.test_obj.units[101].max_health_now  # 被黑的情况
        people = self.test_obj.resource[0]['remain_people']
        self.test_obj.cleanup_phase()
        id_collection = list(self.test_obj.units)
        self.assertTrue(not(101 in id_collection))
        self.assertTrue(
...
class testmove_phase(unittest.TestCase):
    def setUp(self):
        self.test_obj = gamemain.GameMain()  # __init__()写好后加入参数
        self.test_obj.units[100] = unit.UnitObject(100, 0, 'meat', (1, 1), test_buff)  # 加入一个编号为100的单位

    def test_speed(self):  # 速度测试
        for building in self.test_obj.buildings:
            building.position = (50, 50)
        i = self.test_obj.units[100].max_speed_now
        self.test_obj.units[100].position = (1, 1)  # 初始位置为（1,1）
        self.test_obj.move_instr_0 = [[100, 1, 1 + i]]
        self.test_obj.move_phase()
        self.assertTrue(self.test_obj.units[100].position[1] == 1 + i)  # 测试合法移动是否有效

    def test_wrong_speed(self):  # 非法速度测试
        for building in self.test_obj.buildings:
            building.position = (50, 50)
        i = self.test_obj.units[100].max_speed_now + 1
        self.test_obj.units[100].position = (1, 1)  # 初始位置为（1,1）
        self.test_obj.move_instr_0 = [[100, 1, 1 + i]]
        self.test_obj.move_phase()
        self.assertTrue(self.test_obj.units[100].position[1] != 1 + i)  # 测试非法移动是否无效
    ...
    def test_move_to_building(self):  # 非法移动测试
        ...
        self.test_obj.units[101] = unit.UnitObject(100, 0, 'meat', (49, 50), test_buff)
        self.test_obj.units[102] = unit.UnitObject(101, 0, 'meat', (49, 50), test_buff)
...
class testproduce_phase(unittest.TestCase):
    ...
    def testhacker(self):  # 测试是否能够生产黑客（仅测试单位数量是否增加）
        for building in self.test_obj.buildings:
            if building.Get_type_name() == 'hack_lab':
                ...
                building.flag = 0;
        tem_unit = len(self.test_obj.units)
        self.test_obj.produce_phase()
        self.assertTrue(len(self.test_obj.units) != tem_unit)

    def test_no_remain_people(self):  # 生产101个小鲜肉，是否仅生产出100个(人口测试)
        for i in range(101):
            self.test_obj.produce_instr_1.append(self.test_obj.hqs[1].unit_id)
        tem_unit = len(self.test_obj.units)
        self.test_obj.produce_phase()
        self.assertTrue(len(self.test_obj.units) == tem_unit + 100)
    ...
    def test_max_amount(self):  # 生产多个核子坦克，鹰式战斗机，改造人，是否仅生产三个（数量限制测试）
        ...
        tem_unit = len(self.test_obj.units)
        self.test_obj.produce_phase()
        self.assertTrue(len(self.test_obj.units) == tem_unit + 3)
