...
void player_main(void)
{
	...
	vector<Unit>meats;	
		...
		if (init1)
		{
			...
			for (int i = 9; i <= 15; i++)
			{
				if (netural_units[i].size() > 0)
				{
					cap_target.push_back(netural_units[i][0]);
				}
				
			}

			//if (netural_units[BID_LAB].size() > 0)
			//{
			//	cap_target.push_back(netural_units[BID_LAB][0]);
			//}
		...
		for (int i = BID_LAB; i <= AIRCRAFT_LAB; i++)
		{
			if (my_units[i].size() != 0)
			{
				for (int j = 0; j < my_units[i].size(); j++)
				{
					if (myResourse.remain_people_1 >= origin_attribute[i - 7][PEOPLE_COST]&& my_units[i - 7].size()<10)
					{
